...
/**
 * The designers' schedule (Plan.md, Nächste Schritte 17): the screen's
 * playlists, the rules that switch between them in order of precedence, and
 * a preview of any day. Edits go into the draft like every other change –
 * undoable, and stored with „Speichern".
 */
...
const props = defineProps<{ calendars: Calendar[]; timeZone: string; appointments: Appointment[] }>();
const emit = defineEmits<{ close: []; edit: [playlistId: string] }>();
...
function addPlaylist(): void {
    const id = editor.addPlaylist(`Playlist ${editor.playlists.length + 1}`);
    // The new playlist gets edited in the slide list; the dialog stays for the rule.
...
function calendarName(id: number): string {
    return props.calendars.find((c) => c.id === id)?.name ?? `Kalender ${id}`;
}

// Preview

const PREVIEW_DAYS = 90; // the preview loads appointments this far ahead (usePreview)
const today = zonedDateKey(new Date(), props.timeZone);
const lastDay = zonedDateKey(new Date(Date.now() + (PREVIEW_DAYS - 1) * 86_400_000), props.timeZone);
const previewDate = ref(today);
const now = zonedParts(new Date(), props.timeZone);
const previewMinute = ref(Math.floor((now.hour * 60 + now.minute) / 15) * 15);

const day = computed(() => {
    const [year, month, dayOfMonth] = (/^\d{4}-\d{2}-\d{2}$/.test(previewDate.value) ? previewDate.value : today)
        .split('-')
...
const weekdayName = computed(() => {
    const noon = zonedTimeToInstant({ ...day.value, hour: 12 }, props.timeZone);
    return WEEKDAYS[zonedParts(noon, props.timeZone).weekday - 1]!.long;
});

const timeline = computed(() =>
    editor.draft ? dayTimeline(editor.draft.screen, day.value, props.timeZone, props.appointments) : [],
);
...
<template>
    <div class="d-dialog-backdrop" @click.self="emit('close')">
        <section
            ref="dialog"
        ...
        >
            <header class="head">
                <h2 id="schedule-title">Zeitplan</h2>
                <button class="d-btn d-btn--icon" type="button" aria-label="Schließen" @click="emit('close')">
                    <Icon name="close" />
                </button>
            </header>
            <p class="muted intro">
                Welche Playlist wann läuft. Passt keine Regel, läuft die <strong>Standard-Playlist</strong>; passen
                mehrere, gilt die <strong>obere</strong>. Übernommen wird mit „Speichern".
            </p>
            ...
            <ul class="playlists">
                <li v-for="p in editor.playlists" :key="p.id" data-testid="schedule-playlist">
                    ...
                    <button class="d-btn" type="button" data-testid="playlist-edit" @click="emit('edit', p.id)">
                        Slides bearbeiten
                    </button>
                    <button
            ...
            <div class="d-dialog-actions">
                <button class="d-btn d-btn--primary" type="button" data-testid="schedule-done" @click="emit('close')">
                    Fertig
                </button>
            </div>
        </section>
