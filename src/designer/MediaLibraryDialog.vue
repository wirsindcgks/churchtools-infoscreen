...
async function upload(files: FileList | File[] | null): Promise<void> {
    ...
    try {
        ...
        const docs = await library.upload(prepared, props.screen);
        await reload();
...
<template>
    <div class="backdrop" role="dialog" aria-modal="true" aria-label="Mediathek" @click.self="emit('close')">
        ...
        >
            <header>
                <h2>Mediathek</h2>
                <span class="hint">Bilder liegen im Wiki-Bereich „Infoscreen", Seite <code>{{ screen.slug }}</code>.</span>
                <span class="spacer" />
                <button class="d-btn d-btn--primary" type="button" :disabled="!!busy || loading" @click="input?.click()">
                    Bilder hochladen
                </button>
            ...
            <div v-else-if="items.length" class="grid">
                ...
                >
                    <button class="pick" type="button" :title="`${item.name} verwenden`" @click="choose(item)">
