...
**Weg A trägt keinen Dauerbetrieb (G32, 2026-09-25):** Auch „Angemeldet bleiben" hält nur 24 Stunden, danach bindet ChurchTools unser Skript nicht mehr ein. **ChurchTools nimmt aber `login_token` in der Adresse beim Laden an (G9)** und nimmt ihn dabei aus der Adresse – unser Skript sieht ihn nie. **Nächster Schritt, Weg B:** Startadresse mit `login_token` im Query (meldet bei jedem Laden an) **und** im Fragment (`#…`, übersteht die Weiterleitung, liest der Player); der Player lädt sich selbst nur noch über die Adresse mit Token neu, sodass das nächtliche Neuladen die Sitzung täglich erneuert. Dazu ein Adress-Generator in den Screen-Einstellungen (Punkt 15): Benutzername und Passwort des Geräte-Kontos → `POST /login/token` → Adresse, ohne das Passwort zu speichern. Zu messen: dass der Browser das Fragment über die `302` mitnimmt (Spezifikation sagt ja), in Chromium und WebKit.
